    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const item = await db
          ...
          .findOne({ _id: ObjectID(id) }); //there is no cursor that returns

        //! ObjectID(id)  : _id is a type of objectId, basically converts id parameter to object
        resolve(item);
        client.close();
      } catch (error) {
        reject(error);
  ...
  function add(item) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        // ! whenever you miss an await - it will put out promise<pending> insted of doing promise.then, awaits lets you return
